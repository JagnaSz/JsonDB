...
public class Schema {
	...
	public String toString() {
		...
		for (Map.Entry map : schema.entrySet()) {
			...
			builder.append("\t\"")
					.append(map.getKey().toString().replace("\"", ""))
					.append("\": ");
			builder.append("[");
	...
	@SuppressWarnings("rawtypes")
	public void parseValues(StringBuilder builder,
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			for (Map.Entry values : lista.get(i).entrySet()) {
				...
				if (!values.getKey().toString().equals("")) {
					builder.append("\t\t\t");
					// builder.append(values.getKey().toString()).append(": ").append(values.getValue().toString().isEmpty()
					// ? "null" : values.getValue().toString());
					builder.append(values.getKey().toString().isEmpty() ? "null"
							: values.getKey().toString());
					builder.append(":");
					builder.append(values.getValue().toString().isEmpty() ? "null"
	 ...
	 * @throws SQLException
	 */
	public boolean insert(String table, List<String> columns,
			List<String> values) throws SQLException {
		if (!schema.containsKey(table))
			throw new SQLException("table: " + table + " doesn't exist");
		List<String> validColumnes = new ArrayList<String>(schema.get(table)
				.get(0).keySet());
		if (columns == null) {
		...
		for (int i = 0; i < validColumnes.size(); i++) {
			if (columns.contains(validColumnes.get(i)))
				record.put(validColumnes.get(i),
						values.get(columns.indexOf(validColumnes.get(i))));
			else
				record.put(validColumnes.get(i), "");
		}
	 ...
	 * @throws SQLException
	 */
	public boolean createTable(String tableName, String[] columnes)
			throws SQLException {
	...
	public String select(String[] columns, String table) throws SQLException {
		...
		if ("*".equals(columns[0]))
			columns = schema
					...
					.keySet()
					.toArray(
							new String[schema.get(table).get(0).keySet().size()]);
	...
	public int update(String table, List<Map<String, String>> newValues,
			Map<String, String> whereItems) throws SQLException {
		...
		for (Map<String, String> record : records) {
			for (String column : whereItems.keySet()) {
				...
				if (record.containsKey(column)
						&& record.containsValue(whereItems.get(column)))
			...
			for (Map<String, String> map : newValues) {
				for (String key : map.keySet()) {
					if (!record.containsKey(key))
						throw new SQLException("Column " + key
								+ " doesn't exist");
					if (record.containsKey(key))
